...
Для показа без сервера собирается отдельная версия фронтенда: данные лежат в браузере
посетителя (localStorage), сервер и база не нужны. Публикуется автоматически при каждом
пуше в `main` через `.github/workflows/demo.yml`.

Ссылка: https://massonedrew-dotcom.github.io/phoenixCRM/

Один раз включите Pages: Settings → Pages → Build and deployment → Source: **GitHub Actions**.

Чем демо отличается от рабочей системы: вход пускает по одному логину без пароля
(проверять пароль некому), изменения видит только тот, кто их сделал, загруженные
фото хранятся в его же браузере. Для работы агентства нужен сервер, см. разделы выше.

Собрать демо локально:

```bash
cd frontend
VITE_DEMO=1 VITE_BASE=/phoenixCRM/ npm run build
npx vite preview --port 4173 --base /phoenixCRM/
```
